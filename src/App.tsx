...
// --- Types ---
type Page = 'home' | 'seller_home' | 'sales' | 'map' | 'detail' | 'payment' | 'complete' | 'reservations' | 'history' | 'register' | 'my' | 'login' | 'wishlist';
    ...
    <div className="flex justify-center bg-gray-100 min-h-screen">
      <div className="w-full max-w-[390px] h-screen bg-white relative overflow-y-auto flex flex-col shadow-2xl">
        ...
        <div className={`flex-1 overflow-y-auto ${currentPage !== 'detail' && currentPage !== 'payment' && currentPage !== 'complete' && currentPage !== 'login' ? 'pb-20' : ''}`}>
          {currentPage === 'login' && <LoginPage onLogin={(role) => { setUserRole(role); navigateTo(role === 'SELLER' ? 'seller_home' : 'home'); }} />}
...
function ReservationsPage({ userRole }: { userRole?: 'USER' | 'SELLER' }) {
...
function RegisterPage({ onNavigate }: { onNavigate?: (page: Page) => void }) {
  ...
  const handleSubmit = () => {
    ...
    if (window.confirm("이대로 등록하시겠습니까?")) {
      ...
      // 돌아갈 페이지는 뒤로가기 처럼 이전 상태에 따라 분기할 수 있지만, 홈으로 통일하거나 판매페이지로 이동
...
function MyPage({ onNavigate, userRole }: { onNavigate: (page: Page) => void, userRole?: 'USER' | 'SELLER' }) {
  if (userRole === 'SELLER') {
    return (
  ...
  return (
    <div className="flex flex-col min-h-full bg-gray-50 relative pb-20">
      ...
      <header className="bg-gray-50 p-4 flex justify-between items-center sticky top-0 z-20 shrink-0 min-h-[64px]">
        ...
        <div className="w-8"></div> {/* Placeholder to balance the flex space-between */}
      </header>
      ...
      <div className="px-5 py-2 flex flex-col gap-5">
        ...
        {/* Expected Deposit */}
        <div className="bg-[#f2f6ff] rounded-2xl p-5 flex justify-between items-center cursor-pointer transition-colors active:bg-blue-100 mt-2">
          <span className="font-bold text-[15px] text-gray-800">입금 예정 금액</span>
          <div className="flex items-center gap-0.5 font-bold text-[18px] text-blue-600">
            <span>249,180원</span>
            <span className="text-blue-500 text-xl font-normal ml-0.5">›</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mt-1">
          <div className="bg-white rounded-2xl p-5 shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-gray-100/80 cursor-pointer transition-shadow">
            <div className="text-gray-600 font-bold mb-2.5 text-sm">어제 주문금액</div>
            <div className="flex items-center justify-between mb-1">
              <span className="font-extrabold text-[19px] text-gray-900">1,520,180원</span>
              <span className="text-gray-400 font-light text-xl -mt-1">›</span>
            </div>
            <div className="text-emerald-500 font-extrabold text-[13px] tracking-tight">+ 34,000원</div>
          </div>
          
          <div className="bg-white rounded-2xl p-5 shadow-[0_2px_10px_rgba(0,0,0,0.03)] border border-gray-100/80 cursor-pointer transition-shadow">
            <div className="text-gray-600 font-bold mb-2.5 text-sm">어제 주문수</div>
            <div className="flex items-center justify-between mb-1">
              <span className="font-extrabold text-[19px] text-gray-900">21건</span>
              <span className="text-gray-400 font-light text-xl -mt-1">›</span>
            </div>
            <div className="text-orange-500 font-extrabold text-[13px] tracking-tight">- 2건</div>
          </div>
        </div>
        ...
        <div className="pb-10">
          ...
          <div className="flex flex-col gap-1.5 px-1 relative">
            ...
            {/* Fade effect at the bottom to mimic scrollable continuing list */}
